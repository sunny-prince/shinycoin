    ...
    txdelegate(new TxViewDelegate())
{
    ...
    ui->labelGenerated->setFont(QFont("Monospace", -1, QFont::Bold));
    ui->labelGenerated->setToolTip(tr("Total of mined coins that have yet to mature, and do not yet count toward the current balance"));
    ui->labelGenerated->setTextInteractionFlags(Qt::TextSelectableByMouse|Qt::TextSelectableByKeyboard);
...
void OverviewPage::setBalance(qint64 balance, qint64 stake, qint64 generated, qint64 unconfirmedBalance)
{
    ...
    ui->labelGenerated->setText(BitcoinUnits::formatWithUnit(unit, generated));
...
void OverviewPage::setModel(WalletModel *model)
{
    ...
    if(model)
    {
        ...
        setBalance(model->getBalance(), model->getStake(), model->getGenerated(), model->getUnconfirmedBalance());
...
void OverviewPage::displayUnitChanged()
{
    if(!model || !model->getOptionsModel())
        return;
    if(currentBalance != -1)
        setBalance(currentBalance, currentStake, currentGenerated, currentUnconfirmedBalance);

...
CRamhogThreadPool *pramhogPool = NULL;

map<uint256, CBlockIndex*> mapBlockIndex;
set<pair<COutPoint, unsigned int> > setStakeSeen;

std::string strGenesisTimestampString;
unsigned int nGenesisTime = 0;
unsigned int nGenesisNonce = 0;
...
bool AddOrphanTx(const CDataStream& vMsg, bool fDebugOrphanTx=true)
{
    CTransaction tx;
    CDataStream(vMsg) >> tx;
...
bool CBlock::MinerGetPoWHash(uint256 &outHash) const
{
    ...
    return pramhogPool->ramhog((const uint8_t *)pbegin, (pend - pbegin),
                               (uint8_t *)&outHash, sizeof(outHash),
                               true);
}
...
int64 GetNextProofOfWorkReward(const CBlockIndex* pPrev, bool fAccountForPoS)
{
    ...
    if (nPoWHeight < 576)
    {
        nFullSubsidy = COIN + 399 * COIN * (nPoWHeight - 1) * (nPoWHeight - 1) / 575 / 575;
    }
    else
    {
        nFullSubsidy = (400 * COIN * 26280 * 26280) / (nPoWHeight * nPoWHeight);
    }
...
static const int64 nTargetSpacingWorkMax = 60 * STAKE_TARGET_SPACING; // 2-hour
...
const CBlockIndex* GetLastBlockIndex2(const CBlockIndex* pindex, bool fProofOfStake)
{
...
int64 GetTypeHeight(const CBlockIndex *pindex, bool fProofOfStake)
{
    if (!pindex)
        return 0;
    
    if (fProofOfStake)
        return pindex->nHeight - pindex->nPoWHeight;
    else
        return pindex->nPoWHeight;
}

unsigned int static GetNextTargetRequired(const CBlockIndex *pBlockLastSolved, bool fProofOfStake)
{
    ...
    int64 nLastBlockTime = pBlockLastSolved->GetBlockTime();
    CBigNum bnPastTargetAverage;
    const CBlockIndex *pLastBlockReading = NULL;
    while (pBlockReading && GetTypeHeight(pBlockReading, fProofOfStake) > 1 && nCountBlocks < nPastBlocksMax)
    {
        if (nCountBlocks == 0)
        {
            bnPastTargetAverage.SetCompact(pBlockReading->nBits);
        }
        else
        {
            CBigNum bnTarget;
            bnTarget.SetCompact(pBlockReading->nBits);
            bnPastTargetAverage = ((bnPastTargetAverage * nCountBlocks) + bnTarget) / (nCountBlocks + 1);
        }

        nActualTimespan += (nLastBlockTime - pBlockReading->GetBlockTime());
        nLastBlockTime = pBlockReading->GetBlockTime();
        pLastBlockReading = pBlockReading;
    ...
    if (fProofOfStake)
    {
        nTargetTimespan = nCountBlocks * STAKE_TARGET_SPACING;
    }
    else
    {
        int64 nCountStakeBlocks = GetTypeHeight(pBlockLastSolved, true) - GetTypeHeight(pLastBlockReading, true);
        if (fPrint)
            printf("    nCountStakeBlocks=%"PRI64d"\n", nCountStakeBlocks);
        nTargetTimespan = nCountBlocks * WORK_TARGET_SPACING + nCountStakeBlocks * STAKE_TARGET_SPACING;
        if (nTargetTimespan > nCountBlocks * nTargetSpacingWorkMax)
            nTargetTimespan = nCountBlocks * nTargetSpacingWorkMax;
    }
    
    double dMaxAdjust = 1.0 + 2.0 * (double)nCountBlocks / (double)nPastBlocksMax;
    
    if (fPrint)
    {
        printf("    nCountBlocks=%"PRI64d"\n    bnPastTargetAverage=0x%s\n    nActualTimespan=%"PRI64d"\n",
               nCountBlocks, bnPastTargetAverage.GetHex(64).c_str(), nActualTimespan);
        printf("    nTargetTimespan=%"PRI64d"\n    dMaxAdjust=%.4f\n", nTargetTimespan, dMaxAdjust);
    }
    
    if (nActualTimespan < nTargetTimespan / dMaxAdjust)
        nActualTimespan = (int64)(nTargetTimespan / dMaxAdjust);
    if (nActualTimespan > nTargetTimespan * dMaxAdjust)
        nActualTimespan = (int64)(nTargetTimespan * dMaxAdjust);
    
    CBigNum bnNewTarget = bnPastTargetAverage * nActualTimespan / nTargetTimespan;
    
    if (bnNewTarget > bnProofOfWorkLimit)
        bnNewTarget = bnProofOfWorkLimit;
...
bool CTransaction::_procUndoTxInfos(CTxDB &txdb, const bool undoing, uint256 blockHash, unsigned int nTxInBlock) {
    ...
    for (int i=undoing ? (vin.size()-1) : 0; undoing ? (i >= 0) : (i < vin.size()); undoing ? (i--) : (i++)) {
        ...
        for (int j=undoing ? (txin.infos.size()-1) : 0; undoing ? (j >= 0) : (j < txin.infos.size()); undoing ? (j--) : (j++))
        {
            ...
            std::string reason;
            TxInfoRecord record(addr, txinfo, blockHash, nTxInBlock, i, j);
            if (!(undoing ? ptxinfoStore->Undo(record, reason)
                          : ptxinfoStore->Process(record, reason)))
                return error(("%sTxInfos(): Could not process <%s: %s>: " + reason).c_str(),
                             undoing ? "Undo" : "Process",
...
bool CTransaction::UndoTxInfos(CTxDB &txdb, uint256 blockHash, unsigned int nTxInBlock)
{
    return _procUndoTxInfos(txdb, true, blockHash, nTxInBlock);
}
bool CTransaction::ProcessTxInfos(CTxDB &txdb, uint256 blockHash, unsigned int nTxInBlock)
{
    return _procUndoTxInfos(txdb, false, blockHash, nTxInBlock);
}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            if (!tx.ProcessTxInfos(txdb, GetIDHash(), nTx))
...
bool Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    {
        if (pindex->GetBlockIDHash() == Checkpoints::hashSyncCheckpoint)
            return error("Reorganize() : Cannot disconnect sync checkpoint");
...
// ppcoin: total coin age spent in transaction, in the unit of coin-seconds.
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nShinyScratchpads = TEST_SHINY_PADS;
        nShinyHashChunks = TEST_SHINY_CHUNKS;
        nShinyHashIterations = TEST_SHINY_ITERS;
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char *pszGenesisTimestampString = strGenesisTimestampString.c_str();
        printf("Timestamp string: %s\n", pszGenesisTimestampString);
        CTransaction txNew;
        txNew.nTime = nGenesisTime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszGenesisTimestampString, (const unsigned char*)pszGenesisTimestampString + strlen(pszGenesisTimestampString));
                ...
                static CCriticalSection cs;
                {
                    ...
                    if (GetTimeMillis() - nHPSTimerStart > 4000)
                    {
                        ...
                        if (GetTime() - nLogTime > 60)
                        {
                            nLogTime = GetTime();
                            printf("%s ", DateTimeStrFormat(GetTime()).c_str());
                            printf("hashmeter %3d CPUs %.4f hash/min\n", vnThreadsRunning[THREAD_MINER], dHashesPerMin);
                        }
                    }
                }
            }

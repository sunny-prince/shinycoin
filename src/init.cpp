...
void Shutdown(void* parg)
{
    ...
    if (fFirstThread)
    {
        ...
        delete pramhogPool; pramhogPool = NULL;
...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          ...
          _("Options:") + "\n" +
            ...
            "  -port=<port>     \t\t  " + _("Listen for connections on <port> (default: 7801 or testnet: 7803)") + "\n" +
            "  -maxconnections=<n>\t  " + _("Maintain at most <n> connections to peers (default: 125)") + "\n" +
            "  -addnode=<ip>    \t  "   + _("Add a node to connect to and attempt to keep the connection open") + "\n" +
            "  -connect=<ip>    \t\t  " + _("Connect only to the specified node") + "\n" +
            "  -listen          \t  "   + _("Accept connections from outside (default: 1)") + "\n" +
            "  -dontadvertise=<ip>\t  " + _("Don't advertise the given IP address to other nodes") + "\n" +
#ifdef QT_GUI
            ...
            "  -superbantime=<n>\t  "   + _("Number of seconds to keep very misbehaving peers from reconnecting (default: 2592000)") + 
            ...
            "  -rpcport=<port>  \t\t  " + _("Listen for JSON-RPC connections on <port> (default: 7802)") + "\n" +
    ...
    BOOST_FOREACH(std::string strAddr, mapMultiArgs["-dontadvertise"])
    {
        CAddress addr(CService(strAddr, GetDefaultPort(), false));
        if (addr.IsValid())
            setAddrDontAdvertise.insert(addr);
    }

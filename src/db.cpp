...
{
    return Write(make_pair(string("blockindex"), blockindex.GetBlockIDHash()), blockindex);
}
...
bool CTxDB::LoadBlockIndex()
{
    ...
    loop
    {
        ...
        if (strType == "blockindex" && !fRequestShutdown)
        {
            ...
            CBlockIndex* pindexNew = InsertBlockIndex(diskindex.GetBlockIDHash());
            ...
            pindexNew->nPoSTotalMint  = diskindex.nPoSTotalMint;
            pindexNew->nPoSDebt       = diskindex.nPoSDebt;
            ...
            if (pindexGenesisBlock == NULL && diskindex.GetBlockIDHash() == hashGenesisBlock)
    ...
    for (CBlockIndex* pindex = pindexBest; pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel>0 && !block.CheckBlock(nCheckLevel > 5))
        {
            printf("LoadBlockIndex() : *** found bad block at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockIDHash().ToString().c_str());
                    ...
                    if (nCheckLevel>3)
                    {
                        BOOST_FOREACH(const CDiskTxPos &txpos, txindex.vSpent)
                        {
                            if (!txpos.IsNull())
                            {
                                ...
                                {
                                    printf("LoadBlockIndex(): *** found bad spend at %d, hashBlock=%s, hashTx=%s\n", pindex->nHeight, pindex->GetBlockIDHash().ToString().c_str(), hashTx.ToString().c_str());
                                    pindexFork = pindex->pprev;
                                }

...
std::set<CAddress> setAddrDontAdvertise;
...
bool CNode::Misbehaving(int howmuch)
{
    ...
    {
        int64 banTimeRel = nMisbehavior >= 500 ? GetArg("-superbantime", 60*60*24*30) : GetArg("-bantime", 60*60*24);
        int64 banTime = GetTime()+banTimeRel;
        {
            LOCK(cs_setBanned);
...
void StartNode(void* parg)
{
    ...
    // Map ports with UPnP
    if (!mapArgs.count("-connect") && !fNoListen && fHaveUPnP)
    {
        MapPort(fUseUPnP);
    }

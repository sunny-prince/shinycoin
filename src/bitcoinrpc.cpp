...
Value gethashespermin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "gethashespermin\n"
            "Returns a recent hashes per minute performance measurement while generating.");

    if (GetTimeMillis() - nHPSTimerStart > 8000)
        return (boost::int64_t)0;
    return (boost::int64_t)dHashesPerMin;
}
...
Value estimatehpm(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error("estimatehpm <blocks_back=100>\n"
                            "Estimate the historical hashes/minute of the network");
    
    int blocksBack = 100;
    if (params.size() > 0)
        blocksBack = params[0].get_int();
    
    CBlockIndex *lastPoW = mapBlockIndex[hashBestChain];
    while (lastPoW && !lastPoW->IsProofOfWork())
        lastPoW = lastPoW->pprev;
    
    while (lastPoW && lastPoW->nHeight > 1 && blocksBack > 0)
    {
        ...
        printf("Height %d, target 0x%s, %ds gen = ~%s h/min\n",
               lastPoW->nHeight, bnTarget.GetHex(256/4).c_str(),
               secsTaken,
               ((maxTarget / bnTarget) * 60 / secsTaken).ToString().c_str());
...
Value estimatecoindays(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error("estimatecoindays <blocks_back=100>\n"
                            "Estimate the historical coindays staked of the network");
    
    int blocksBack = 100;
    if (params.size() > 0)
        blocksBack = params[0].get_int();
    
    CBlockIndex *lastPoS = mapBlockIndex[hashBestChain];
    while (lastPoS && !lastPoS->IsProofOfStake())
        lastPoS = lastPoS->pprev;
    
    while (lastPoS && lastPoS->nHeight > 1 && blocksBack > 0)
    {
        CBlockIndex *prevPoS = lastPoS->pprev;
        while (prevPoS && !prevPoS->IsProofOfStake())
        ...
        printf("Height %d, target 0x%s, %ds gen = ~%s coindays\n",
               lastPoS->nHeight, bnTarget.GetHex(256/4).c_str(),
               secsTaken,
               (maxTarget / bnTarget / secsTaken).ToString().c_str());
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    //{ "getnettotals",           &getnettotals,           true },
    ...
    //{ "gettxoutsetinfo",        &gettxoutsetinfo,        true},
    //{ "verifychain",            &verifychain,            true},
    ...
    //{ "decodescript",           &decodescript,           false},
    { "getrawtransaction",      &getrawtransaction,      false},
    { "sendrawtransaction",     &sendrawtransaction,     false},
    //{ "signrawtransaction",     &signrawtransaction,     false},
    
    //{ "createmultisig",         &createmultisig,         true},
    //{ "estimatefee",            &estimatefee,            true},
    //{ "estimatepriority",       &estimatepriority,       true},
    ...
    { "dumpprivkey",            &dumpprivkey,            true},
    //{ "dumpwallet",             &dumpwallet,             true},
    ...
    //{ "getrawchangeaddress",    &getrawchangeaddress,    true},
    { "getreceivedbyaccount",   &getreceivedbyaccount,   false},
    { "getreceivedbyaddress",   &getreceivedbyaddress,   false},
    { "gettransaction",         &gettransaction,         false},
    //{ "getunconfirmedbalance",  &getunconfirmedbalance,  false},
    //{ "getwalletinfo",          &getwalletinfo,          true},
    { "importprivkey",          &importprivkey,          false},
    //{ "importwallet",           &importwallet,           false},
    ...
    { "gethashespermin",        &gethashespermin,        true},
